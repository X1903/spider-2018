...
for i in range(0, len(allcheci)):
    try:
        ...
        # [31]---zy
        zy = thischeci[26]
        print(code + "\t" + fromname + "\t" + toname + "\t" + stime + "\t" + atime + "\t" + str(zy) + "\t" + str(
            ze) + "\t" + str(yz) + "\t" + str(wz))
...
urllib.request.urlretrieve(yzmurl, "./yzm.png")
...
checkdata = urllib.parse.urlencode({
    ...
    "passengerTicketStr": "O,0,1," + str(nameall[thisno]) + ",1," + str(idall[thisno]) + "," + str(
        mobileall[thisno]) + ",N",
...
getquedata = "train_date=" + str(thisgmtdate) + "+00%3A00%3A00+GMT%2B0800&train_no=" + traindata[thiscode][
    0] + "&stationTrainCode=" + thiscode + "&seatType=1&fromStationTelecode=" + traindata[thiscode][
                 1] + "&toStationTelecode=" + traindata[thiscode][
                 2] + "&leftTicket=" + leftstr2 + "&purpose_codes=00&train_location=P4&_json_att=&REPEAT_SUBMIT_TOKEN=" + str(
    token)
getdata = getquedata.encode('utf-8')
req10 = urllib.request.Request(getqueurl, getdata)
req10.add_header('User-Agent',
                 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/38.0.2125.122 Safari/537.36 SE 2.X MetaSr 1.0')
...
req11.add_header('User-Agent',
                 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/38.0.2125.122 Safari/537.36 SE 2.X MetaSr 1.0')
req11data = urllib.request.urlopen(req11).read().decode("utf-8", "ignore")
print("配置确认提交完成，即将进行下一步")
# 总请求4-确认步骤2-获取orderid
getorderidurl = "https://kyfw.12306.cn/otn/confirmPassenger/queryOrderWaitTime?tourFlag=dc&_json_att=&REPEAT_SUBMIT_TOKEN=" + str(
    token)
